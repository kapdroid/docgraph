...
type NodeSet struct {
	...
	// Exclude lists doublestar patterns to drop from the glob matches — a real repo must keep
	// fixtures/vendored docs out of its own graph. Patterns are whole-path matched, so use a leading
	// "**/" to match at any depth: "**/testdata/**", "**/vendor/**" (a bare "testdata/**" matches only
	// a root-level testdata/).
